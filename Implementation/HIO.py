...
def fienup_phase_retrieval(angle,mag, mask=None, beta=0.8,steps=200, mode='hybrid', verbose=True):
    """
    ...
    # if read==1:
    #   angle_guess=angle
    #   y_hat = mag*np.exp(1j*angle_guess)
    # else:  
    #   global initialization
    #   initialization=np.random.rand(*mag.shape)
    #   y_hat = mag*np.exp(1j*2*np.pi*initialization)
    y_hat = mag*np.exp(1j*angle)  
